...
export default class ChatRoom extends EventSubscriber {
  constructor(sockets, name, options) {
    ...
    this.io = sockets.io.of(`/${this.name}`)
    this.subs.push(userCounter(this.io))
  ...
  handleJoin(socket, videoType) {
    this.listenTo(socket, 'exit', () => this.handleExit(socket))
  ...
  handleExit(socket) {
    debug('exit', this.sockets.userIdMap.get(socket))
